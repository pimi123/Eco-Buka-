...
<template>
  <section v-if="sectionVisible" class="bg-white py-10 sm:py-12 lg:py-14" aria-label="Featured video promotion">
    <div class="container-shell">
      <h2 v-if="content.title" class="mb-5 text-2xl font-black leading-tight text-ink sm:text-3xl">
        {{ content.title }}
      </h2>

      <div v-if="loading" class="h-[420px] animate-pulse rounded-2xl bg-mist sm:h-[480px] lg:h-[540px]" />

      <div v-else class="relative min-h-[520px] overflow-hidden rounded-2xl bg-ink shadow-panel sm:min-h-[420px] lg:min-h-[520px]">
        <img
        ...
        <div class="absolute inset-0 bg-gradient-to-r from-black/82 via-black/50 to-black/12" />

        <div class="relative z-10 flex min-h-[520px] items-center px-5 py-10 sm:min-h-[420px] sm:px-10 lg:min-h-[520px] lg:px-14">
          <div class="max-w-xl text-white">
            <p v-if="content.promoLabel" class="text-sm font-black uppercase tracking-wide text-orange-400 sm:text-base">
              {{ content.promoLabel }}
            </p>
            <h3 class="mt-3 text-3xl font-black leading-tight sm:text-4xl lg:text-5xl">
              {{ content.heading }}
            </h3>
